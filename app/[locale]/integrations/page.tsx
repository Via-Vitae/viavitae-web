...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-4 py-16">
      <Breadcrumbs label={t('nav.breadcrumb')} crumbs={[{ name: t('nav.home'), path: '/' }, { name: t('nav.integrations'), path: '/integrations' }]} />
      <JsonLd data={{ '@context': 'https://schema.org', '@type': 'ItemList', itemListElement: itemList }} />
      <header>
      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {INTEGRATIONS.map((id) => (
          <li key={id} className="flex flex-col gap-2 rounded-md border border-border bg-surface-raised p-5">
            <span className="text-body-lg font-semibold text-text-primary">{t(`integrations.${id}.name`)}</span>
            <span className="text-body-md text-text-secondary">{t(`integrations.${id}.description`)}</span>
            <span className="mt-2 text-body-sm text-text-muted">{t(`integrations.${id}.compliance`)}</span>
          </li>
        ))}
      </ul>
    </div>
