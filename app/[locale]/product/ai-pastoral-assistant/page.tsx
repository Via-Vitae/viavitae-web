...
const PRODUCT_ID: ProductId = 'ai-pastoral-assistant';
...
export default async function ProductPage({
  ...
  const faq = (t.has(`product.faq.${PRODUCT_ID}`)
    ? (t.raw(`product.faq.${PRODUCT_ID}`) as FaqItem[])
    : []) satisfies FaqItem[];

  return (
    <>
      <JsonLd data={[productJsonLd({ product, name, description: subtitle, tierName: (id) => t('tiers.' + id + '.name') }), ...(faq.length ? [faqJsonLd(faq)] : [])]} />
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-12 px-4 py-12">
        ...
        <section aria-labelledby="features-heading">
          <h2 id="features-heading" className="text-heading-2 text-text-primary">{t('product.featuresHeading')}</h2>
          <ul className="mt-4 grid gap-3 sm:grid-cols-2">
            {features.map((feature) => (
              <li key={feature} className="flex items-start gap-2 text-body-md text-text-secondary">
                <span aria-hidden="true" className="text-state-success">&#10003;</span>
                {feature}
              </li>
            ))}
          </ul>
        ...
        <section aria-labelledby="demo-heading">
          <h2 id="demo-heading" className="text-heading-2 text-text-primary">{t('product.demoHeading')}</h2>
          <div className="mt-4">
            <DemoEmbed demoType={product.demoType} title={t('product.demoTitle', { product: name })} />
          </div>
        </section>

        <section aria-labelledby="pricing-heading">
          <h2 id="pricing-heading" className="text-heading-2 text-text-primary">{t('product.pricingHeading')}</h2>
          <div className="mt-4">
            <TierTable />
          </div>
        </section>
        ...
        {faq.length > 0 && (
          <section aria-labelledby="faq-heading">
            <h2 id="faq-heading" className="text-heading-2 text-text-primary">{t('product.faqHeading')}</h2>
            <div className="mt-4">
        ...
        <section aria-labelledby="cta-heading" className="rounded-md border border-border bg-surface-raised p-6">
          <h2 id="cta-heading" className="text-heading-3 text-text-primary">{t('product.ctaHeading')}</h2>
