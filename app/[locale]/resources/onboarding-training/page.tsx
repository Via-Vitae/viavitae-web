...
const PATH = "/resources/onboarding-training";
const NS = "onboarding";
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const links = t.has(`${NS}.links`)
    ? (t.raw(`${NS}.links`) as Array<{ label: string; href: string }>)
    : [];
    ...
    <div className="flex flex-col gap-8">
      <Breadcrumbs
        ...
        crumbs={[
          ...
          { name: t("nav.resources"), path: "/resources/blog" },
      ...
      {links.length > 0 && (
        <ul className="flex flex-col gap-2">
          {links.map((link) => (
            <li key={link.href}>
              <Link href={link.href} className="text-body-md text-text-link underline">
                {link.label}
              </Link>
            </li>
          ))}
