...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const docs = (await Promise.all(slugs.map((slug) => getDoc<GuideFrontMatter>('guides', loc, slug))))
    .filter((doc): doc is { frontMatter: GuideFrontMatter; body: string } => Boolean(doc && !doc.frontMatter.draft))
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {docs.length === 0 ? (
        <p className="rounded-md border border-border bg-surface-sunken p-6 text-body-md text-text-secondary">{t('guides.empty')}</p>
      ) : (
        <div className="flex flex-col gap-3">
          {docs.map((doc) => (
            <details key={doc.frontMatter.slug} className="rounded-md border border-border bg-surface-raised p-5">
              <summary className="cursor-pointer text-body-lg font-semibold text-text-primary">
                {doc.frontMatter.title}
                <span className="ml-2 text-body-sm font-normal text-text-muted">{doc.frontMatter.topic}</span>
              </summary>
