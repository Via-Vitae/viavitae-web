...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const docs = (await Promise.all(slugs.map((slug) => getDoc<NewsFrontMatter>('news', loc, slug))))
    .filter((doc): doc is { frontMatter: NewsFrontMatter; body: string } => Boolean(doc && !doc.frontMatter.draft))
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {docs.length === 0 ? (
        <p className="rounded-md border border-border bg-surface-sunken p-6 text-body-md text-text-secondary">{t('news.empty')}</p>
      ) : (
        <ul className="flex flex-col gap-4">
          {docs.map((doc) => (
            <li key={doc.frontMatter.slug} className="flex flex-col gap-1 rounded-md border border-border bg-surface-raised p-5">
              ...
              <span className="text-body-md text-text-secondary">{doc.frontMatter.description}</span>
              {doc.frontMatter.source && (
                <a href={doc.frontMatter.source} className="mt-1 text-body-sm text-text-link underline" target="_blank" rel="noopener noreferrer">
                  {t('news.readSource')}
                </a>
              )}
            </li>
