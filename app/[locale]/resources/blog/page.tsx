...
export default async function Page({ params, searchParams }: Props) {
  ...
  const docs = (await Promise.all(slugs.map((slug) => getDoc<BlogFrontMatter>("blog", loc, slug))))
  ...
  const categories = Array.from(
    new Set(
      (await Promise.all(slugs.map((s) => getDoc<BlogFrontMatter>("blog", loc, s))))
    ...
    <div className="flex flex-col gap-8">
      ...
      {categories.length > 0 && (
        <nav aria-label={t("blog.categoryFilter")} className="flex flex-wrap gap-2">
          <Link
            href={hrefFor(1)}
            className="rounded-sm border border-border px-3 py-1 text-body-sm"
          >
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((doc) => (
            <li key={doc.frontMatter.slug}>
              <Link
                href={`/resources/blog/${doc.frontMatter.slug}`}
                className="flex h-full flex-col gap-2 rounded-md border border-border bg-surface-raised p-5 hover:shadow-md motion-reduce:transition-none"
              >
      ...
      {pageCount > 1 && (
        <nav aria-label={t("blog.pagination")} className="flex items-center gap-3">
          {page > 1 && (
            <Link
              href={hrefFor(page - 1, query.category)}
              className="text-body-md text-text-link underline"
            >
              {t("blog.prev")}
            </Link>
          )}
          <span className="text-body-sm text-text-muted">
            {t("blog.pageOf", { page, pageCount })}
          </span>
          {page < pageCount && (
            <Link
              href={hrefFor(page + 1, query.category)}
