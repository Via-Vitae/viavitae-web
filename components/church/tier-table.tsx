      ...
      <table className="w-full border-collapse text-body-md">
        ...
        <thead>
          <tr className="sticky top-0 z-10 bg-surface-raised">
            <th scope="col" className="border-b border-border p-3 text-left font-semibold">{t('featureColumn')}</th>
            {TIER_ORDER.map((tierId) => (
              <th key={tierId} scope="col" className="border-b border-border p-3 text-center">
                <span className="block text-body-lg font-semibold text-text-primary">{tRoot(`tiers.${tierId}.name`)}</span>
                <span className="block text-display-sm text-text-brand">{formatEur(TIERS[tierId].priceEur, locale)}</span>
        ...
        <tbody>
          {FEATURES.map((feature) => (
            <tr key={feature.key}>
              <th scope="row" className="border-b border-border-divider p-3 text-left font-normal text-text-secondary">
                {t(`features.${feature.key}`)}
              </th>
              {TIER_ORDER.map((tierId) => (
                <td key={tierId} className="border-b border-border-divider p-3 text-center">
                  {feature.byTier[tierId] ? (
                    <span aria-label={t('included')} className="text-state-success">&#10003;</span>
                  ) : (
                    <span aria-label={t('notIncluded')} className="text-text-disabled">&ndash;</span>
                  )}
                </td>
              ))}
            </tr>
          ...
          <tr>
            <th scope="row" className="p-3 text-left font-normal text-text-secondary">{t('maintenance')}</th>
            <td colSpan={TIER_ORDER.length} className="p-3 text-center text-text-primary">
              {formatEur(MAINTENANCE_EUR_MONTH.min, locale)}&ndash;{formatEur(MAINTENANCE_EUR_MONTH.max, locale)} / {t('perMonth')}
            </td>
          </tr>
        </tbody>
      </table>
