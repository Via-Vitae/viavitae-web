...
export function Accordion({ items, className, labelledBy }: AccordionProps) {
  ...
  return (
    <div className={cn('divide-y divide-border-divider border-y border-border-divider', className)} aria-labelledby={labelledBy}>
      {items.map((item) => {
          ...
          <div key={item.id}>
            <h3>
              ...
              >
                ...
                <span aria-hidden="true" className={cn('shrink-0 transition-transform motion-reduce:transition-none', expanded && 'rotate-180')}>
                  &#9662;
                </span>
              </button>
            </h3>
