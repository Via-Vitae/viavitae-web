    ...
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      >
            ...
            {GLOBAL_NAV.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.href}
                  onClick={onClose}
                  className="block rounded-sm px-2 py-2 text-body-md text-text-primary hover:bg-surface-sunken"
                >
                  {t(item.labelKey)}
                </Link>
              </li>
        ...
        <nav aria-label={t("nav.ribbonLabel")}>
          <p className="mb-1 px-2 text-body-sm font-semibold uppercase tracking-wide text-text-muted">
            {t("nav.products")}
          </p>
          <ul className="flex flex-col gap-1">
            {CHURCH_RIBBON.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.href}
                  onClick={onClose}
                  className="block rounded-sm px-2 py-1.5 text-body-sm text-text-secondary hover:bg-surface-sunken"
                >
                  {t(item.labelKey)}
                </Link>
              </li>
        ...
        <div className="mt-auto flex flex-col gap-2 border-t border-border-divider pt-4">
          <Link
            ...
            onClick={onClose}
            className="rounded-sm bg-gold-500 px-4 py-2 text-center text-body-md font-medium text-text-on-gold"
          >
          ...
          <Link
            ...
            onClick={onClose}
            className="rounded-sm border border-border px-4 py-2 text-center text-body-md font-medium text-text-primary"
          >
