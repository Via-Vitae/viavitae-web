...
const CONTRACTS = [
  { file: 'docs/contracts/assessment.openapi.yaml', path: '/assessment', operationId: 'submitAssessment' },
...
for (const { file, path, operationId } of CONTRACTS) {
  ...
  for (const p of Object.keys(paths)) {
    if (p.startsWith('/v1/')) {
      errors.push(`${file}: path "${p}" must not include the /v1 prefix (server URL already ends in /v1)`);
    }
  }
  ...
  if (op.operationId !== operationId) {
    errors.push(`${file}: POST ${path} operationId should be "${operationId}", found "${op.operationId}"`);
  }
  if (!op.responses || Object.keys(op.responses).length === 0) {
    errors.push(`${file}: POST ${path} declares no responses`);
  }

  console.log(`OK ${file}: openapi ${doc.openapi}, info.version ${doc.info?.version}, POST ${path} (${operationId})`);
}
