...
try {
  for (const { spec, out } of CONTRACTS) {
    ...
    if (!existsSync(committedPath)) {
      problems.push(`${out} is not committed. Run \`pnpm generate:types\` and commit the output (ADR-WEB-005).`);
      continue;
    }
    if (readFileSync(committedPath, 'utf8') !== fresh) {
      problems.push(`${out} is stale (spec changed without regenerating, or the file was hand-edited). Run \`pnpm generate:types\` and commit.`);
    }
  }
} finally {
