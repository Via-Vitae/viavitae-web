...
function flatten(value: Json, prefix: string, out: Map<string, Json>): void {
  if (Array.isArray(value)) {
    ...
    value.forEach((element, index) => {
      if (element !== null && typeof element === 'object') flatten(element, `${prefix}.${index}`, out);
    });
    return;
  }
...
for (const locale of ['lt', 'ru'] as const) {
  ...
  for (const key of typeMismatch) {
    problems.push(`${locale}: type mismatch for "${key}" (en=${typeOf(en.get(key))}, ${locale}=${typeOf(other.get(key))})`);
  }
  for (const key of untranslated) problems.push(`${locale}: untranslated (empty) value for "${key}"`);

  if (!missing.length && !extra.length && !typeMismatch.length && !untranslated.length) {
    console.log(`OK ${locale}: ${other.size} keys in parity with en (${en.size} keys), no empty values`);
  }
}
