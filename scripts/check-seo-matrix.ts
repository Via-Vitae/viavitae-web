...
for (const product of CATALOG) {
  ...
  if (!paths.has(productPath)) problems.push(`missing matrix entry for catalogue product: ${productPath}`);
}
...
for (const page of matrix.pages) {
  if (!hasKey(en, page.titleKey)) problems.push(`${page.path}: titleKey "${page.titleKey}" missing in messages/en.json`);
  if (!hasKey(en, page.descriptionKey)) {
    problems.push(`${page.path}: descriptionKey "${page.descriptionKey}" missing in messages/en.json`);
  }
}
...
console.log(`SEO matrix OK: ${matrix.pages.length} entries, ${navHrefs.size} nav hrefs covered, all title/description keys resolve.`);
