...
const definesHeader = (source, header) => new RegExp(`key:\\s*['"]${header}['"]`, "i").test(source);
...
if (definesHeader(nextConfig, "Content-Security-Policy")) {
  errors.push(
    `${NEXT_CONFIG} defines a Content-Security-Policy header; CSP must live only in ${MIDDLEWARE} (ADR-WEB-002).`,
  );
}
...
if (declaredStatic.length === 0) {
  errors.push(
    `${NEXT_CONFIG} defines no static security headers; expected HSTS, X-Content-Type-Options, etc.`,
  );
}
...
if (!setsHeader(middleware, "Content-Security-Policy")) {
  errors.push(
    `${MIDDLEWARE} does not set Content-Security-Policy; the nonce-bearing CSP must be set here (ADR-WEB-002).`,
  );
}
...
for (const header of STATIC_HEADERS) {
  if (setsHeader(middleware, header)) {
    errors.push(
      `${MIDDLEWARE} sets static header "${header}"; static headers belong in ${NEXT_CONFIG} (ADR-WEB-002).`,
    );
  }
}
