...
describe('assessmentRequestSchema (server)', () => {
  ...
  it('rejects consent_privacy that is not literally true (DPIA-001 R7)', () => {
    expect(assessmentRequestSchema.safeParse({ ...valid, consent_privacy: false }).success).toBe(false);
  });
  ...
  it('rejects a missing or malformed contact email', () => {
    ...
    expect(assessmentRequestSchema.safeParse(rest).success).toBe(false);
    expect(assessmentRequestSchema.safeParse({ ...valid, contact_email: 'not-an-email' }).success).toBe(false);
  });
  ...
  it('caps goals at five and rejects unknown organisation types', () => {
    const six = ['new_website', 'ecommerce', 'donations', 'crm_integration', 'ai_assistant', 'cemetery_map'];
    expect(assessmentRequestSchema.safeParse({ ...valid, goals: six }).success).toBe(false);
    expect(assessmentRequestSchema.safeParse({ ...valid, organization_type: 'spaceport' }).success).toBe(false);
  });
  ...
  it('allows optional fields to be empty strings or a valid URL only', () => {
    expect(assessmentRequestSchema.safeParse({ ...valid, organization_name: '' }).success).toBe(true);
    expect(assessmentRequestSchema.safeParse({ ...valid, current_site_url: '' }).success).toBe(true);
    expect(assessmentRequestSchema.safeParse({ ...valid, current_site_url: 'https://parish.lt' }).success).toBe(true);
    expect(assessmentRequestSchema.safeParse({ ...valid, current_site_url: 'not a url' }).success).toBe(false);
  });
