...
test.describe('assessment funnel', () => {
  test.beforeEach(async ({ page }) => {
    await page.route('**/api/assessment', (route) =>
      route.fulfill({ status: 202, contentType: 'application/json', body: JSON.stringify(ACCEPTED) }),
    );
  ...
  test('rejects an invalid email with a field-level error', async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: /I agree to the processing of my contact details/ }).check();
    await page.getByRole('button', { name: 'Submit request' }).click();
